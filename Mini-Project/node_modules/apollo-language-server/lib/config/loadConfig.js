...
async function loadConfig({ configPath, configFileName, requireConfig = false, name, type }) {
    ...
    try {
        ...
        throw new Error(`A config file failed to load with options: ${JSON.stringify(arguments[0])}.
      The error was: ${error}`);
    }
    if (configPath && !loadedConfig) {
        throw new Error(`A config file failed to load at '${configPath}'. This is likely because this file is empty or malformed. For more information, please refer to: https://bit.ly/2ByILPj`);
    }
    ...
    if (!loadedConfig ||
        ...
        loadedConfig = {
            ...
            config: Object.assign({}, (loadedConfig && loadedConfig.config), (projectType === "client"
                ? {
                    client: Object.assign({}, config_1.DefaultConfigBase, (loadedConfig && loadedConfig.config.client), { service: serviceName })
                }
                : {
                    service: Object.assign({}, config_1.DefaultConfigBase, (loadedConfig && loadedConfig.config.service), { name: serviceName })
                }))
        };
    }
    let { config, filepath } = loadedConfig;
    if (config.client)
        config = fp_1.merge({ client: config_1.DefaultClientConfig }, config);
    if (config.service)
        config = fp_1.merge({ service: config_1.DefaultServiceConfig }, config);
    if (engineConfig)
        config = fp_1.merge(engineConfig, config);
    config = fp_1.merge({ engine: config_1.DefaultEngineConfig }, config);
