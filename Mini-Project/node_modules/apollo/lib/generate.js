...
function generate(document, schema, outputPath, only, target, tagName, nextToSources, options) {
    ...
    if (target === "swift") {
        ...
        if (nextToSources ||
            ...
            const globalSourcePath = options.globalTypesFile || path_1.default.join(outputPath, "globalTypes.ts");
...
function writeGeneratedFiles(generatedFiles, outputDirectory) {
    for (const [fileName, generatedFile] of Object.entries(generatedFiles)) {
        localfs_1.fs.writeFileSync(path_1.default.join(outputDirectory, fileName), generatedFile.output);
    }
}
